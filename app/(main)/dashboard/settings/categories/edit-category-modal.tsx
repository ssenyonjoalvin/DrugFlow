...
export function EditCategoryModal({
  ...
  useEffect(() => {
    if (open && category) handledSuccess.current = false;
  }, [open, category?.id]);
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-zinc-200 bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-zinc-800">Edit Category</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form action={formAction} className="mt-6 space-y-4">
          ...
          <div>
            <label htmlFor="edit-categoryName" className="mb-1 block text-sm font-medium text-zinc-700">
              Category Name
            </label>
            <input
          ...
          <div>
            <label htmlFor="edit-categoryDescription" className="mb-1 block text-sm font-medium text-zinc-700">
              Description
            </label>
            <textarea
