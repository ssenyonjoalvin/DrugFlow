...
export function DeleteUnitMeasureModal({
  open,
  unit,
  onClose,
  onConfirm,
  ...
  open: boolean;
  unit: UnitForDelete | null;
  onClose: () => void;
  onConfirm: (unitId: string) => Promise<boolean>;
}) {
  ...
  async function handleYes() {
    ...
    setIsDeleting(true);
    try {
      const success = await onConfirm(unit.id);
      if (success) onClose();
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border border-zinc-200 bg-white p-6 shadow-xl">
        <h2 className="text-lg font-bold text-zinc-800">Delete unit measure?</h2>
